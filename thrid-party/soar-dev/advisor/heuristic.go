	...
	* 所有其他情况下，两个参数都会被转换为浮点数再进行比较
	...
	for _, cond := range conditions {
		...
		case *sqlparser.ComparisonExpr:
			...
			// 补全列信息
			colList = CompleteColumnsInfo(idxAdv, colList)
			...
			if len(colList) == 2 {
				...
				for _, col := range colList {
					if col.Table == "" {
						common.Log.Warning("Column %s not exists, trace_id:%s", col.Name, idxAdv.TraceID)
						needBreak = true
					}

					if col.DataType == "" {
						common.Log.Warning("Can't get column %s data type, trace_id:%s", col.Name, idxAdv.TraceID)
						needBreak = true
					}
				...
				common.Log.Debug("DataType: `%s`.`%s` (%s) VS `%s`.`%s` (%s), trace_id:%s",
					colList[0].Table, colList[0].Name, type1,
					colList[1].Table, colList[1].Name, type2, idxAdv.TraceID)
				...
				// 检查字符集不一致导致的隐式数据转换
				common.Log.Debug("Charset: `%s`.`%s` (%s) VS `%s`.`%s` (%s), trace_id:%s",
					colList[0].Table, colList[0].Name, colList[0].Character,
					colList[1].Table, colList[1].Name, colList[1].Character, idxAdv.TraceID)
				...
				common.Log.Debug("Collation: `%s`.`%s` (%s) VS `%s`.`%s` (%s), trace_id:%s",
					colList[0].Table, colList[0].Name, colList[0].Collation,
					colList[1].Table, colList[1].Name, colList[1].Collation, idxAdv.TraceID)
			...
			for _, val := range values {
				if colList[0].DataType == "" {
					common.Log.Warn("Can't get %s data type, trace_id:%s", colList[0].Name, idxAdv.TraceID)
					break
				}
				...
				if isCovered {
					if colList[0].Table == "" {
						common.Log.Warning("Column %s not exists, trace_id:%s", colList[0].Name, idxAdv.TraceID)
						continue
					}

					c := fmt.Sprintf("The definition of column %s in the %s table is %s instead of %s.",
...
func (idxAdv *IndexAdvisor) RuleGroupByConst() Rule {
	...
	//这里你的release_year已经都是2006了根据他分组没意义 但是这里只是用到了ast 也没有用到table info 循环是在检查groupby的字段在where里面不能是单值匹配
	//可以是age>18 and age<20 group by age 但是不能是age=18 因为只有一种情况就是age=18 group by age没有意义
	...
	} //这个循环是根据ast 拿出group by和where eq两列 名字一样db一样table一样 这里的eq就是等值查询 都一样说明就违反了GRP.001的rule
	return rule
}
...
func (q *Query4Audit) RuleSumNPE() Rule {
	var rule = q.RuleOK()
	fingerprint := query.Fingerprint(q.Query)
	// TODO: https://smart-slowquery/thrid-party/soar-dev/issues/143
...
func (idxAdv *IndexAdvisor) RuleUpdatePrimaryKey() Rule {
	...
	case *sqlparser.Update:
		...
		common.LogIfError(err, "trace_id:%s", idxAdv.TraceID)
		setColumns = CompleteColumnsInfo(idxAdv, setColumns)
		for _, col := range setColumns {
			idxMeta := idxAdv.IndexMeta[col.DB][col.Table]
			if idxMeta == nil {
				return rule
			}
