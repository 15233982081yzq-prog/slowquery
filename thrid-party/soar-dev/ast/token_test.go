...
func TestSplitStatement(t *testing.T) {
	...
	bufs := [][]byte{
		...
		// https://smart-slowquery/thrid-party/soar-dev/issues/66
		[]byte(`/*comment*/`),  // 8
		[]byte(`/*comment*/;`), // 9
		[]byte(`--`),           // 10
		[]byte(`-- comment`),   // 11
		[]byte(`# comment`),    // 12
		// https://smart-slowquery/thrid-party/soar-dev/issues/116
		[]byte(`select
*
-- comment
from tb
...
select col from tb where col = 1;`), // 17
		// https://smart-slowquery/thrid-party/soar-dev/issues/120
		[]byte(`
-- comment
select col from tb;
select col from tb;
