...
baseUrl = "http://10.129.120.132:30083/rds/smart/v1/api/non-live/slowquery/query_list"


local databases = {"test1","test2","test3"}
local orderBy = {"total_time","count","avg_time"}
...
function request()
    ...
    -- print("Generated Query_List URL: " .. full_url)  -- 打印生成的URL
...
function build_query_map()
    ...
    params["db_env"] = env
    params["order_by"] = random_orderBy(orderBy)
    params["page"] = page
...
function random_orderBy(orderBy)
    local random_index = math.random(#orderBy)
    return orderBy[random_index]
end
